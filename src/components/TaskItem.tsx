...
//           style={styles.iconButton}
//           onPress={() => {
//             navigation.navigate("EditTask", { taskId: index })
//           }}
//         >
//           <Ionicons name="pencil-outline" size={20} color="#000" />
//         </TouchableOpacity>
...
interface TaskItemProps {
  task: any;
  index: number;  // Get the index passed as prop
  navigation: any;
  loadTasks: () => void; // Function to refresh task list after deletion
}

const TaskItem: React.FC<TaskItemProps> = ({ task, index, navigation, loadTasks }) => {
  // Function to handle task deletion
  const handleDelete = async () => {
    Alert.alert(
      "Delete Task",
      ...
      [
        ...
        },
        {
          ...
          onPress: async () => {
            ...
            const updatedTasks = tasksArray.filter((_, i) => i !== index); // Remove task by index
  ...
  return (
    <View style={styles.buttonsContainer}>
      ...
      <TouchableOpacity
        style={styles.iconButton}
        onPress={() => {
          navigation.navigate("EditTask", { taskId: index }); // Navigate to EditTask with taskId
        }}
      >
        <Ionicons name="pencil-outline" size={20} color="#000" />
      </TouchableOpacity>
